...
def write_audit_log(
    ...
    safe_run_id = _escape_sql_literal(str(run_id))
    safe_status = _escape_sql_literal(str(status))
    safe_metrics = _escape_sql_literal(metrics_json)
    safe_issues = _escape_sql_literal(issues_json)
    spark.sql(
        f"""
    ...
    spark.sql(
        f"INSERT INTO {table} VALUES (current_timestamp(), '{safe_run_id}', '{safe_status}', '{safe_metrics}', '{safe_issues}')"
    )
