...
def derive_logic(ast_json: str) -> str:
    ...
    Enriches the inferred logic with lineage hints and connector-aware inputs so the
    LLM receives grounded join/filter context without shipping the full AST.
    """
    try:
    ...
    connectors = ast.get("connectors") or []
    ...
    for idx, conn in enumerate(connectors):
        if not isinstance(conn, dict):
            continue
        src = conn.get("from_instance")
        tgt = conn.get("to_instance")
        inbound.setdefault(tgt, []).append(conn)
        outbound.setdefault(src, []).append(conn)
        if idx < 300:
            lineage.append(
                {
    ...
    if len(connectors) > len(lineage):
        lineage.append({"note": f"{len(connectors) - len(lineage)} lineage edges truncated"})
    if lineage:
        logic["lineage"] = lineage
    ...
    for tr in ast.get("transformations", []) or []:
        ...
        inputs = sorted({c.get("from_instance") for c in inbound.get(name, []) if c.get("from_instance")})
        outputs = sorted({c.get("to_instance") for c in outbound.get(name, []) if c.get("to_instance")})

        def _entry(base: Dict[str, object]) -> Dict[str, object]:
            if name:
                base.setdefault("transformation", name)
            if inputs:
                base["inputs"] = inputs
            if outputs:
        ...
        if isinstance(attrs, dict):
            for key in sorted(attrs.keys()):
                if len(compact_attrs) >= 12:
                    break
                val = attrs.get(key)
                if val:
                    compact_attrs[key] = val
        ...
        for k, v in (attrs or {}).items():
            ...
            if "cond" in kv or "filter" in kv:
                logic["filters"].append(_entry({"condition": v, "attr": k}))
            if "join" in kv:
                logic["joins"].append(_entry({"detail": f"{k}: {v}"}))
            if "lookup" in kv or "lkp" in kv:
                logic["lookups"].append(_entry({"detail": f"{k}: {v}"}))
            if "group" in kv or "agg" in kv:
                logic["aggregations"].append(_entry({"detail": f"{k}: {v}"}))
            if "update strategy" in kv or "dd_" in kv:
                logic["update_strategy"].append(_entry({"detail": f"{k}: {v}"}))

        for f in fields:
            expr = f.get("expression")
            if expr:
                logic.setdefault("expressions", []).append(
                    _entry({"field": f.get("name"), "expr": expr})
                )

        if ttype.startswith("filter"):
            entry = _entry({"attrs": compact_attrs})
            logic["filters"].append(entry)

        if ttype.startswith("router"):
            logic["routers"].append(_entry({"attrs": compact_attrs}))

        if "lookup" in ttype or (name or "").lower().startswith("lkp"):
            defaults = [
                {"field": f.get("name"), "default": f.get("expression")}
                for f in fields
                if f.get("expression") and "default" in (f.get("expression") or "").lower()
            ]
            entry = _entry({"attrs": compact_attrs})
            if defaults:
                entry["defaults"] = defaults[:5]
            logic["lookups"].append(entry)

        if ttype.startswith("agg") or "aggregator" in ttype:
            agg_funcs = []
            for f in fields:
                expr = (f.get("expression") or "").lower()
                if any(tok in expr for tok in ["sum(", "avg(", "count(", "min(", "max("]):
                    agg_funcs.append({"field": f.get("name"), "expr": f.get("expression")})
                if len(agg_funcs) >= 8:
                    break
            entry = _entry({"attrs": compact_attrs})
            if agg_funcs:
                entry["metrics"] = agg_funcs
            logic["aggregations"].append(entry)

        if "update" in ttype:
            entry = _entry({"attrs": compact_attrs})
            dd_actions = {k: attrs[k] for k in attrs if k.upper().startswith("DD_")}
            if dd_actions:
                entry["actions"] = dd_actions
            logic["update_strategy"].append(entry)

        if any(token in ttype for token in ["sequence", "surrogate", "generator"]) or (name or "").lower().startswith("seq"):
            logic["sequences"].append(_entry({"attrs": compact_attrs}))

        if "join" in ttype:
            join_inputs = {}
            for conn in inbound.get(name, []) or []:
                src = conn.get("from_instance")
                port = conn.get("to_field")
                field = conn.get("from_field")
                if src and field:
                    join_inputs.setdefault(src, []).append({"field": field, "port": port})
            entry = _entry({"attrs": compact_attrs})
            if join_inputs:
                entry["joins"] = {
                    src: items[:6]
                    for src, items in join_inputs.items()
                }
            logic["joins"].append(entry)
...
def validator(
    ...
    if merge_keys_hint:
        missing_merge_keys = [k for k in merge_keys_hint if k and k not in py and k not in sql_lc]
        if missing_merge_keys:
            issues.append("MERGE keys not referenced in PySpark/SQL: " + ', '.join(missing_merge_keys))
    if part_cols_hint:
        missing_parts = [c for c in part_cols_hint if c and c not in py and c not in sql_lc]
        if missing_parts:
            tips.append("Partition columns not clearly used: " + ', '.join(missing_parts))
...
def get_llm_from_env() -> ChatOpenAI:
    """Return a ChatOpenAI client configured from environment variables."""
    model = os.getenv("LLM_MODEL") or os.getenv("OPENAI_MODEL") or "gpt-4o"
    ...
    if base:
        try:
            return ChatOpenAI(base_url=base, **kwargs)
        except TypeError:
            try:
                return ChatOpenAI(openai_api_base=base, **kwargs)
            except TypeError:
                pass
    try:
